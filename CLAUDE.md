...
| `transparent_panels` | appearance | Render dashboard panels with no background, border, or title-band fill so components float on the canvas. Default false. Set as a `data-transparent-panels` attribute on `:root` by `App.jsx`. A dashboard can OVERRIDE this via `settings.panel_background` (`""` inherit / `solid` / `transparent`), which the viewer reflects as a separate `data-panel-background` root attribute — the two are combined in `DashboardGrid.scss` so the override wins regardless of effect ordering (the global is set asynchronously after identity resolves). Chrome only — no layout/geometry change. Applies on next page load. |
