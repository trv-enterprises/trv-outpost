...
func init() {
	...
	RegisterComponentType(ComponentTypeInfo{
		...
		ConfigSchema: []ConfigField{
			...
			{Name: "valueSourceUnit", Type: "select", Required: false, Default: "none", Options: []string{"none", "k", "M", "G", "T", "Ki", "Mi", "Gi", "Ti"}, Description: "What the stored number already is, so \"compact\" scales from the right magnitude. \"none\" = base units. Decimal k/M/G/T are ×1000 (disk, network); binary Ki/Mi/Gi/Ti are ×1024 (memory, filesystem). A megabytes column holding 123456 with \"M\" renders \"123.5G\". Set valueUnit to the BASE unit (\"B\", not \"MB\")."},
			{Name: "valueType", Type: "select", Required: false, Default: "auto", Options: []string{"auto", "number", "text"}, Description: "Which family of options applies. \"auto\" (default) detects it from the data and is almost always right — set it explicitly only when the sample is empty, mixed, or a stream hasn't produced a record yet. \"text\" renders the value as a string and ignores the numeric format/decimals/unit."},
			...
			{Name: "valueThresholdTarget", Type: "select", Required: false, Default: "text", Options: []string{"text", "background", "both"}, Description: "Where a matched threshold/rule color lands. \"text\" recolors the value only. \"background\" fills the tile and derives a readable text color. \"both\" fills and colors the text. When nothing matches, the tile falls back to valueBackground."},
