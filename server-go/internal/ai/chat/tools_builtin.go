...
func chartDataMappingSchema() map[string]interface{} {
	return map[string]interface{}{
		...
		"properties": map[string]interface{}{
			...
			"y_axis_label":             map[string]interface{}{"type": "string", "description": "AXIS label for the LEFT y axis — rendered at the TOP of the axis. Works on single- AND dual-axis charts, and is especially useful with the legend OFF, where nothing else identifies each axis. NOT a series/legend label; use y_axis_labels for those."},
			"y_axis_label_right":       map[string]interface{}{"type": "string", "description": "AXIS label for the RIGHT y axis, rendered at the TOP of it. Dual-axis only (multiple_y_axis: true); ignored otherwise. Pair with y_axis_label to name both sides."},
