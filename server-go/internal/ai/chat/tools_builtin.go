...
func chartDataMappingSchema() map[string]interface{} {
	return map[string]interface{}{
		...
		"properties": map[string]interface{}{
			...
			"accumulator_mode":         map[string]interface{}{"type": "boolean", "description": "Line/area only. When true, plot each point's DELTA from the previous (value[i]-value[i-1]) instead of the raw value — for monotonically-increasing counters (odometers, packet/request totals, kWh meters). Prefer this over custom code or a SQL LAG()/Prometheus rate(). First point renders as a gap. Default false."},
			"accumulator_reset_policy": map[string]interface{}{"type": "string", "enum": []string{"drop_negative", "clamp_zero", "keep_negative"}, "description": "Counter-reset (negative delta) handling when accumulator_mode is on. 'drop_negative' (default): gap/break the line. 'clamp_zero': emit 0. 'keep_negative': raw negative delta. Ignored when accumulator_mode is off."},
