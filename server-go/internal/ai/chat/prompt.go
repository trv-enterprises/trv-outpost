...
3. **Outline the WHOLE structure as text BEFORE the first create_component — this is a hard gate.** Write out the complete plan first: every section, every component in each section (name + chart_type + which column(s)), and its panel rectangle {x,y,w,h}. Group into logical sections (e.g. "CPU & MEMORY", "DISK & FILESYSTEM", "NETWORK") and aim to fill the canvas. Do NOT create any component until the full plan is written — creating a first batch and then planning the rest leads to re-work, re-created components, and inconsistent layouts. Plan the entire component set in one outline, then create them all in one pass.
