...
- **Custom-code components MUST still set the ` + "`title`" + ` field AND render it — custom code does not get a title for free.** Spec-driven charts get their title band from the server's codegen; custom code does not, so a custom-code component with no title field and no title div renders as a bare chart with no header (unlike every canonical chart on the dashboard). Two requirements, both mandatory: (1) pass a concise, human-readable ` + "`title`" + ` in the same ` + "`create_component`" + ` / ` + "`update_component`" + ` call (don't rely on the name→title fallback — the name is an internal slug); (2) in the component_code, READ that title from the ` + "`config`" + ` prop and render it as an HTML div ABOVE the chart, exactly like the canonical layout — never via ` + "`option.title`" + ` (ECharts' title forces an ugly gap). Pattern: ` + "`const Component = ({ data, config }) => { const title = config?.title || ''; return (<div style={{height:'100%',display:'flex',flexDirection:'column'}}>{title && <div style={{fontSize:'0.875rem',fontWeight:600,color:CARBON_COLORS.text,padding:'0.25rem 0.5rem'}}>{title}</div>}<div style={{flex:1,minHeight:0}}><ReactECharts option={option} style={{height:'100%'}} /></div></div>); }`" + `. Reading from ` + "`config?.title`" + ` (not a hardcoded string) keeps the header in sync when the user renames the component.
