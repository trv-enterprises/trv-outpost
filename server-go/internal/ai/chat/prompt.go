...
5c. **BOTH ` + "`cols`" + ` AND ` + "`rows`" + ` ARE HARD CEILINGS — not guidelines. Every panel must satisfy ` + "`x + w ≤ cols`" + ` AND ` + "`y + h ≤ rows`" + `.** ` + "`cols`/`rows`" + ` (from step 2's cell budget) are the LAST usable column/row; anything past either is clipped off-canvas (right edge or bottom) and invisible. Check each axis with the same discipline:
    ...
    - **Height (whole dashboard):** every panel's ` + "`y + h ≤ rows`" + `, and the final ` + "`cursorY`" + ` after the last row MUST equal ` + "`rows`" + ` (fill the vertical space — do not stop at row 18 of 27, and do not run past row 27). ADD UP all row heights (headers + chart rows) BEFORE create_dashboard. Overshoot fixes: shorter chart rows (time-series reads fine at 9-11 cells), two sections side-by-side, 2-cell headers, or drop the lowest-priority section. Undershoot fixes: grow chart rows toward the high end, add breathing room, or include another useful section — until the total equals ` + "`rows`" + `.
   State BOTH the per-row width sum and the total height arithmetic in your plan, hitting the budget exactly on each ("row1: 5 tiles → 10+10+11+11+11 = 53 = cols ✓; heights 6 + 2+11 + 2+12 + 2+12 = 47 > 27 → too tall, drop to two side-by-side chart rows → 6 + 2+10 + 2+10 = 30 → trim to 9 → 6+2+9+2+9 = 27 = rows ✓"). A dashboard that overflows ` + "`cols`/`rows`" + ` (clipped) OR underfills them (dead space, scales poorly) is a build defect.
