...
5. **Size panels by role — but the row budget (5c) wins.** Gauges and number tiles are small (typically 8-14 cells wide × 8-10 tall). Time-series line/area charts are wide (24+ cells wide × 9-14 tall). Don't ship a dashboard of uniform 12×12 panels — that's a sign you skipped step 3. These are starting heights; when many sections stack, scale heights DOWN (toward the low end, ~9) so the total fits ` + "`rows`" + ` — a slightly shorter chart that's visible beats a taller one that's clipped off-canvas. **Number tiles: give every number tile on the dashboard the SAME height (so a single font size fits them all), then set ` + "`numberSize`" + ` from that height (≈13px per cell — a 6-cell tile → ~80px) and use that SAME numberSize for every number tile so they read uniformly. The default 120 overflows a 6-cell tile. Drop decimals on values >99.**
...
5c. **THE TOTAL MUST FIT IN ` + "`rows`" + ` — this is a hard ceiling, not a guideline.** ` + "`rows`" + ` (from step 2's cell budget) is the LAST usable cell row; every panel must satisfy ` + "`y + h ≤ rows`" + `, so your final ` + "`cursorY`" + ` after the last row must be ` + "`≤ rows`" + `. Before you call create_dashboard, ADD UP every row's height (header heights + chart-row heights) and confirm the sum ≤ ` + "`rows`" + `. If it overflows (the common failure — stacking 5-6 full sections at 12-18 cells each blows past a 27-row HD canvas), you MUST make it fit — do NOT just place panels past the boundary (everything below ` + "`rows`" + ` is invisible/clipped). To fit: (a) shrink chart-row heights (a time-series reads fine at 9-11 cells, not 16-18); (b) put two sections' worth of content side-by-side in one row instead of stacking (e.g. two 26-wide charts in one row uses the 53-col width instead of two stacked rows); (c) shrink header strips to 2 cells; (d) as a last resort, drop the lowest-priority section. State the row arithmetic in your plan ("6 + 2+10 + 2+10 = 30 > 27 → shrink chart rows to 9: 6 + 2+9 + 2+9 = 28, still over → …") so you catch the overflow BEFORE creating, not after. A dashboard whose content extends past ` + "`rows`" + ` is a build defect.
