...
- **filter** — a value the viewer picks (or types), substituted into the query wherever you wrote the ` + "`{{dashboard-variable}}`" + ` token. Author the component query as e.g. ` + "`SELECT ... FROM metrics WHERE site = {{dashboard-variable}}`" + `. Config goes under the ` + "`filter_value`" + ` key (NOT ` + "`filter`" + `): ` + "`filter_value: { value_source: \"connection\"|\"static\"|\"freetext\", options: [...], default_value, value_column, value_table }`" + `. **Prefer ` + "`value_source: \"connection\"`" + `** — it discovers the dropdown options LIVE from ` + "`value_column`" + ` of ` + "`value_table`" + ` so they stay in sync with the data; only use ` + "`static`" + ` (a fixed ` + "`options`" + ` list) when the user explicitly wants a fixed set. AT MOST ONE filter variable per dashboard (the token is a single fixed name). Name it ` + "`\"dashboard-variable\"`" + `.
