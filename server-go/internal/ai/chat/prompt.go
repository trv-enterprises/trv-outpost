...
1. **Probe the data shape.** ` + "`list_connections`" + ` to find the right source, then ` + "`query_connection`" + ` with ` + "`limit: 1`" + ` (or ` + "`sample_mqtt_topic`" + ` for streams) to confirm column names and value types. Don't guess column names — wrong names produce broken charts.
