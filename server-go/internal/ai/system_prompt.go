...
- **light**: Color bulb — power toggle, brightness bar, and a color swatch picker. Sends a composite object ({"state":"ON","brightness":0-254,"color":{"hex":"#rrggbb"}}) rather than a scalar, so it needs a device type whose command sets passthrough_value (e.g. the built-in "zigbee-color-light"). Color is written as hex; Zigbee2MQTT reports it back as CIE {x,y}, which the client converts for display. UI: { label, state_field (default: "state"), show_brightness, show_color }
  - target: MQTT command topic (e.g., "zigbee2mqtt/device_name/set"). State topic is derived by removing "/set" suffix.
- **tile_light**: Compact tile form of **light**. Fill height is brightness and takes the bulb's live color; the color swatch sits on the tile face for one-tap changes. Shows a read-only motion dot when the device also reports ` + "`occupancy`" + ` (motion *driving* the light is an automation concern, not something this control does). UI: { label, icon, state_field, show_color_on_tile }
