...
- **CRITICAL: tool-call self-check.** After EVERY configuration tool call, before telling the user it's done, run this check: "Did the parameters I just passed actually contain a value that addresses what the user asked for?" If no, you have NOT done what the user asked. Examples:
  ...
  - User asks "show this in Fahrenheit" (or psi / mph / km) → set ` + "`update_data_mapping`" + ` ` + "`y_axis_conversions`" + ` = {"<column>": {"dimension":"temperature","from":"c","to":"f"}}. The conversion applies to the DATA, so the axis, thresholds and tooltip all follow. Do NOT convert in the query or in custom code.
...
- **Time-series: plot RAW values — the native renderer scales and labels for you.** A spec-driven ` + "`line`" + `/` + "`area`" + ` chart auto-abbreviates large Y values with ONE shared SI prefix (K/M/G/B) across the whole axis AND the tooltip (issue #159; on by default, ` + "`chartSiPrefixes: false`" + ` opts out). So map the RAW ` + "`bytes_sec`" + ` / ` + "`bytes`" + ` column — do NOT pre-divide by 1024 in the query or in custom code to fake KB/MB. Raw ` + "`rx_bytes_sec`" + ` renders a clean ` + "`50K / 100K / 150K`" + ` axis on its own; ` + "`tooltip.units`" + ` only adds a cosmetic suffix on top and is not needed for scaling. The X axis formats timestamps natively (dated, spaced) — hand-rolled ECharts usually yields a WORSE, cramped, undated time axis. Net: for a plain time-series, native ` + "`line`" + `/` + "`area`" + ` beats ` + "`set_custom_code`" + ` on both axes. Custom code is for an unsupported chart SHAPE, never for unit/axis formatting.
...
| Display a series in a different unit (°C→°F, psi, mph)  | ` + "`update_data_mapping`" + ` with ` + "`y_axis_conversions`" + ` keyed by column name (line/area, per-series) — do NOT do unit math in the query, write custom code, or ask for a second stored column |
...
| control        | ` + "`control_type`" + `  | toggle, button, slider, plug, dimmer, light |
...
- **light**: Colour bulb — power toggle, brightness bar, and a colour swatch picker. Sends a composite object ({"state":"ON","brightness":0-254,"color":{"hex":"#rrggbb"}}) rather than a scalar, so it needs a device type whose command sets passthrough_value (e.g. the built-in "zigbee-color-light"). Colour is written as hex; Zigbee2MQTT reports it back as CIE {x,y}, which the client converts for display. UI: { label, state_field (default: "state"), show_brightness, show_color }
  - target: MQTT command topic (e.g., "zigbee2mqtt/device_name/set"). State topic is derived by removing "/set" suffix.
- **tile_light**: Compact tile form of **light**. Fill height is brightness and takes the bulb's live colour; the colour swatch sits on the tile face for one-tap changes. Shows a read-only motion dot when the device also reports ` + "`occupancy`" + ` (motion *driving* the light is an automation concern, not something this control does). UI: { label, icon, state_field, show_color_on_tile }
...
func controlTypesProse(cat *registry.Catalog) string {
	if cat == nil {
		return "button, toggle, slider, text_input, plug, dimmer, light, garage_door, text_label"
	}
