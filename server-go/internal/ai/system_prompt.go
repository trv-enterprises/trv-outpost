...
- **CRITICAL: Configure first, custom-code last — BUT custom-code IS the right answer when configuration tools can't express the request.** Configuration tools (` + "`update_data_mapping`" + `, ` + "`update_chart_options`" + `, ` + "`update_filters`" + `, ` + "`update_aggregation`" + `, ` + "`update_sliding_window`" + `, ` + "`update_time_bucket`" + `, ` + "`update_control_config`" + `, ` + "`update_display_config`" + `) cover MOST chart changes — column choices, axis formats, legend, sort/limit, banded-bar style + reference levels, sliding windows. ` + "`update_chart_options`" + ` now also covers **y-axis min/max + log scale** (` + "`yAxisRange`" + `), **tooltip** mode/decimals/units (` + "`tooltip`" + `), **downsampling** (` + "`sampling`" + `), the **zoom slider** (` + "`chartShowZoomSlider`" + `), and **threshold coloring** (` + "`yThresholds`" + ` + ` + "`yThresholdRenderMode: \"color_segments\"`" + ` — this is how you change a line's color above/below a value). Use them when they fit. They still do NOT cover everything: there is no tool for an UNCONDITIONAL specific series color (e.g. \"make the line green\" with no threshold), no tool for a fully custom tooltip *formatter function*, no tool for arbitrary ECharts options. Spec-driven charts color series automatically (single = Carbon blue; dual-axis = blue/purple; 3+ = Carbon categorical palette); the only config color control is threshold-based via ` + "`yThresholds`" + `. When the user asks for an unconditional specific color or any other item not in the tool, configuration tools are NOT an option and you must call ` + "`set_custom_code`" + `. Do not call a related-but-wrong configuration tool just to have called something.
...
- **CRITICAL: tool-call self-check.** After EVERY configuration tool call, before telling the user it's done, run this check: "Did the parameters I just passed actually contain a value that addresses what the user asked for?" If no, you have NOT done what the user asked. Examples:
  ...
  - User asks "make the line green" (no condition), you call ` + "`update_chart_options`" + ` with some unrelated param → FAILED. There is no unconditional color param; call ` + "`set_custom_code`" + ` next.
...
6. If the chart type has style/options (e.g. bandedBarStyle, chartSmooth, numberFormat, yAxisRange, yThresholds, etc.), call update_chart_options to set them — field names are the exact camelCase keys in the tool schema. (The only color control is threshold-based via yThresholds; an unconditional specific color needs set_custom_code.)
