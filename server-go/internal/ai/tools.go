		...
		},
		{
			...
			InputSchema: anthropic.ToolInputSchemaParam{
				Properties: map[string]interface{}{
					...
					"y_axis_label":   map[string]interface{}{"type": "string", "description": "AXIS label — rendered vertically along the Y axis (single-axis charts only; ignored when dual-axis is on, where each axis takes its series label). NOT a series/legend label — do not put a series name here; use y_axis_labels for those."},
					"y_axis_labels":  map[string]interface{}{"type": "array", "items": map[string]interface{}{"type": "string"}, "description": "Per-SERIES labels, index-aligned to y_axis — shown in the legend, and on dual-axis charts each side's axis inherits its series label. These name the value-sets, not the axis; the axis label is y_axis_label."},
