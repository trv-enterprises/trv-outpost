		...
		},
		{
			...
			InputSchema: anthropic.ToolInputSchemaParam{
				Properties: map[string]interface{}{
					...
					"y_axis_label":        map[string]interface{}{"type": "string", "description": "AXIS label for the LEFT y axis — rendered at the TOP of the axis. Works on single- AND dual-axis charts. Especially useful with the legend OFF, where nothing else says which axis is which. NOT a series/legend label — do not put a series name here; use y_axis_labels for those."},
					"y_axis_label_right":  map[string]interface{}{"type": "string", "description": "AXIS label for the RIGHT y axis, rendered at the TOP of it. Dual-axis charts only (multiple_y_axis: true) — ignored when there is no right axis. Pair with y_axis_label to name both sides of a two-scale chart."},
					"y_axis_labels":       map[string]interface{}{"type": "array", "items": map[string]interface{}{"type": "string"}, "description": "Per-SERIES labels, index-aligned to y_axis — shown in the legend. These name the value-sets, not the axis; the axis labels are y_axis_label / y_axis_label_right."},
