		...
		},
		{
			...
			InputSchema: anthropic.ToolInputSchemaParam{
				Properties: map[string]interface{}{
					...
					"accumulator_mode": map[string]interface{}{"type": "boolean", "description": "Line/area only. When true, plot each point's DELTA from the previous point (value[i]-value[i-1]) instead of the raw value — the right transform for monotonically-increasing counters (odometers, packet/request totals, kWh meters). Prefer this declarative flag over writing custom code or a SQL LAG()/Prometheus rate(). The first point has no predecessor (rendered as a gap). Default false."},
					"accumulator_reset_policy": map[string]interface{}{"type": "string", "description": "How to handle a counter reset (a negative delta) when accumulator_mode is on. 'drop_negative' (default): emit a gap so the line breaks at the reset. 'clamp_zero': emit 0. 'keep_negative': surface the raw negative delta. Ignored when accumulator_mode is false.", "enum": []string{"drop_negative", "clamp_zero", "keep_negative"}},
