...
| **Transparent Panels** | Appearance | Renders dashboard panels with no background, border, or title-band fill, so components appear to float directly on the dashboard's dark canvas. Off (default) gives the standard raised panel surface. Applies to every dashboard that hasn't overridden it — an individual dashboard can force solid or transparent via **Panel background** in its own [settings](dashboard-settings.md). Panel sizes and positions are unchanged either way. Applies on next page load. |
