---
sidebar_position: 1
slug: /
---

# TRV Outpost User Guide

Welcome to TRV Outpost, a full-stack application for creating, managing, and viewing dynamic data visualization dashboards with real-time data from multiple sources.

## Getting Started

- [Logging In & User Selection](getting-started.md)
- [Application Modes](modes.md)

## View Mode

- [Viewing Dashboards](viewing-dashboards.md)
- [Dashboard Navigation & Controls](viewer-controls.md)

## Editing Dashboards

- [Live Dashboard Editor](dashboard-editor.md)
- [Panel Management](panel-management.md)
- [Dashboard Settings](dashboard-settings.md)

## Components

- [Components Overview](components-overview.md)
- [Creating Components](creating-components.md)
- [Chart Types](chart-types.md)
- [Control Types](control-types.md)
- [Display Types](display-types.md)
- [AI Component Builder](ai-builder.md)

## Connections

- [Connections Overview](connections-overview.md)
- [Connection Types](connection-types.md)

## Sharing & Organization

- [Namespaces](namespaces.md)
- [Export & Import](export-import.md)

## Administration

- [User Management](user-management.md)
- [System Settings](system-settings.md)
- [Device Types](device-types.md)

## Automation & Integration

- [MCP Server (External Agents)](mcp.md)

## Reference

- [Keyboard Shortcuts](keyboard-shortcuts.md)
- [Grid & Layout System](grid-layout.md)
